...
export const rateLimiters = {
    cliLogin: (key: string) =>
        rateLimit(key, { identifier: "cli-login", maxRequests: 10, windowSeconds: 60 }),

    cliLoginStatus: (key: string) =>
        rateLimit(key, { identifier: "cli-login-status", maxRequests: 60, windowSeconds: 60 }),

    tunnelAuth: (key: string) =>
        rateLimit(key, { identifier: "tunnel-auth", maxRequests: 30, windowSeconds: 60 }),

    tunnelRegister: (key: string) =>
        rateLimit(key, { identifier: "tunnel-register", maxRequests: 20, windowSeconds: 60 }),

    tokenExchange: (key: string) =>
        rateLimit(key, { identifier: "token-exchange", maxRequests: 20, windowSeconds: 60 }),

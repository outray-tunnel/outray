...
export const Route = createFileRoute("/$orgSlug/secrets/projects")({
  head: () => ({ meta: [{ title: "Projects - OutRay Secrets" }] }),
...
function ProjectsPage() {
  ...
  return (
    <SecretsPage>
      <SecretsHeader
        title="Projects"
        description="Create one project per application or service, then organize its secret values by environment."
        action={<SecretsButton tone="primary" icon={Add01Icon} onClick={() => setCreating(true)}>New project</SecretsButton>}
      />
      ...
      {resource.loading && !resource.data ? (
        ...
        <SecretsEmptyState icon={Folder01Icon} title="Projects could not be loaded" description={resource.error} action={<SecretsButton onClick={resource.reload}>Try again</SecretsButton>} />
      ) : (resource.data || []).length === 0 ? (
        <SecretsEmptyState
          icon={Folder01Icon}
          title="No projects yet"
          description="Create a project for an application, then add its development, staging, and production environments."
          action={<SecretsButton tone="primary" icon={Add01Icon} onClick={() => setCreating(true)}>Create project</SecretsButton>}
        />
      ) : (
        <>
          <div className="relative max-w-sm">
            ...
            <input className={`${fieldClassName} pl-10`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search projects" aria-label="Search projects" />
          </div>
          {visibleProjects.length === 0 ? (
            <div className="rounded-2xl border border-white/[0.08] px-6 py-14 text-center text-[13px] text-zinc-600">No projects match “{query}”.</div>
          ) : (

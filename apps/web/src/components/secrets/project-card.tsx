...
export function ProjectCard({ orgSlug, project }: { orgSlug: string; project: SecretProject }) {
  return (
    <article className="group flex min-h-52 flex-col rounded-2xl border border-white/[0.085] bg-white/[0.014] p-5 transition-colors hover:border-white/[0.14] hover:bg-white/[0.024]">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          >
            ...
            <HugeiconsIcon icon={ArrowRight01Icon} size={14} strokeWidth={1.7} className="shrink-0 text-zinc-500 transition-transform group-hover:translate-x-0.5 group-hover:text-zinc-400" />
          </Link>
          <p className="mt-1 truncate font-mono text-[13px] text-zinc-500">{project.slug}</p>
        </div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {project.environments.slice(0, 3).map((environment) => (
          <Link
            ...
            params={{ orgSlug, projectSlug: project.slug, environmentSlug: environment.slug }}
        ...
        {project.environments.length > 3 && <SecretsBadge>+{project.environments.length - 3}</SecretsBadge>}
        {project.environments.length === 0 && <span className="text-[13px] text-zinc-500">No environments</span>}
      </div>
      ...
      <div className="mt-auto flex items-center gap-4 border-t border-white/[0.065] pt-4 text-[13px] text-zinc-500">
        ...
        <span className="ml-auto hidden sm:inline">{formatRelativeDate(project.updatedAt)}</span>
      </div>
    </article>
  );
}
